...
- **Session recording:** the same idle-watcher loop tracks each session's start/end, writing one record to `backups/sessions.json` per session (`record_session()`) — see [Realm status page](#realm-status-page) for why this is session-based rather than a fixed-interval sample. Session bounds are keyed on the *real player's* own online status (`real_player_online_count()`), not the aggregate bot+player count the sleep decision above uses — bots take up to 300s to log off after you disconnect, and the idle timer then runs its own threshold on top of that, so using the aggregate count for session bounds would count all of that shutdown overhead as play time (a real 2-3 minute session was recording as 12 minutes before this fix).
- **Per-character progress:** at session start, `fetch_session_start_state()` records level/gold/xp/quests-completed/exploration (`fetch_character_snapshot()`) plus reputation and skill values (`_fetch_guid_keyed_counts()`, shared between `character_reputation` and `character_skills` since both are plain guid+id+value tables) for whichever real (non-`RNDBOT%`) characters are online, keyed by guid. At session end it re-queries those same guids (the real player has already logged out by then, so filtering on `online=1` again would find nothing) and diffs both snapshots, plus a direct timestamp-range query against `character_achievement.date` for achievements earned in between. This is what actually changed to *your* character during the session — level/xp gained, gold earned, quests turned in, achievements earned, reputation gained, profession skill-ups, new areas explored — as opposed to `fetch_economy_snapshot()`'s realm-wide AH numbers above. Session records from before this existed simply don't have a `characters` field; the status page falls back to the old peak-population display for those.
  - **XP gained** is only reported when no level-up happened during the session — the `xp` column resets (down, not to 0) on level-up, so a naive before/after delta across one would be meaningless or even negative. The level-gained line already covers that case.
  - **Money delta** is stored and diffed as raw copper (`characters.money`'s own unit — 1g = 100s = 10000c), not pre-rounded to gold: rounding a 50-silver session down to gold made it vanish entirely as "+0g". The status page's `formatMoneyDelta()` renders only the denominations that actually changed (e.g. "+50s" instead of "+0g" or a misleading "+1g").
  - **Reputation/skill names**: faction and skill-line IDs both come from DBC data this core doesn't import as SQL (same gap as `achievement_dbc`/`lock_dbc` — see [Realm status page](#realm-status-page) below), so this deliberately only reports a *count* of factions gained ("rep with N factions"), never faction names. Skills are the exception: `PROFESSION_NAMES` (mirrored from `status-page/status_page.py`) is a small, hand-verified, unchanging-since-launch mapping for the ~14 profession/secondary skill lines specifically — not DBC-extracted data, just common game knowledge — so those get named. Weapon/defense skill-ups are deliberately excluded from `skills_gained`: they train passively from nearly every kill, which would clutter every session with trivial "+1 Sword Skill" noise instead of the deliberate crafting progress this is meant to surface.
  - **Areas explored** counts newly-set bits in the `exploredZones` bitmask (`count_new_explored_bits()`) between the two snapshots — pure bit arithmetic, no DBC data needed, since it only needs to know *how many* new bits got set, not which map area each bit corresponds to (that mapping lives in `AreaTable.dbc` and isn't attempted here, same reasoning as the faction/skill naming above).
...
**Session history, not a time-sampled trend:** this host only runs 1-2h/day, so a fixed-interval sampler (e.g. every 5 minutes) would spend the vast majority of its points on "asleep" — and worse, a line chart connecting the last point before sleep to the first point after would draw a smooth line implying continuous change through 20+ idle hours that never happened. Instead, `wake-proxy`'s idle-watcher loop (which already knows exactly when a session starts and ends) records one summary per play session to `backups/sessions.json` (see `record_session()` in [`wake-proxy/wake_proxy.py`](./wake-proxy/wake_proxy.py)). `status-page` mounts that same file read-only and renders it as a session list (`/api/sessions`) plus two cumulative stats (total sessions, longest session) rather than a graph.

**Per-character progress, not realm stats:** each session entry shows what *your* character actually did — level gained, gold earned, quests completed, achievements earned — rather than realm-wide numbers like peak population or Auction House economy delta. The latter told you how busy the *realm* was, not what you personally accomplished, which is what this page is actually for on a solo server. See `fetch_character_snapshot()` and `fetch_achievement_counts()` in `wake_proxy.py` for how the per-session diff is computed.
